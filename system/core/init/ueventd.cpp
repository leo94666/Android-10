...
int ueventd_main(int argc, char** argv) {
     ...
     */
    umask(000);
    //初始化内核日志，位于节点/dev/kmsg，此时logd和logcat进程还没有起来
    //采用kernel的log系统，打开设备节点/dev/kmsg，那么通过cat /dev/kmsg来获取内核log
    android::base::InitLogging(argv, &android::base::KernelLogger);

    LOG(INFO) << "ueventd started!";
    LOG(INFO) << "=====liyang=============================================ueventd started!";
    ...
    // for SIGCHLD above.
    //在最后一次调用waitpid和为上面SIGCHLD设置SIG_IGN之间退出的获取和挂起子级
